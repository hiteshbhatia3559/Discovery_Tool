...
class FinestraMain(Gtk.Window):
    ...
    def _insert_packet(self, method, p):
        ...
        if (Ether in p) and (p[Ether].src is not None) and (p[Ether].src != "00:00:00:00:00:00") and (
                self.liststore[t][2] != "") and (p[Ether].src not in self.liststore[t][2]):
            print
            "From " + self.liststore[t][2] + " to " + p[Ether].src
